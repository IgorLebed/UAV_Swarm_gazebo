...
class MavrosTestCommon(unittest.TestCase):
    ...
    def setUp(self, uav_id, scout_id):
        ...
        try:
            rospy.wait_for_service('/bomber' + str(self.uav_id) + '/mavros/param/get', service_timeout)
            rospy.wait_for_service('/bomber' + str(self.uav_id) + '/mavros/cmd/arming', service_timeout)
            rospy.wait_for_service('/bomber' + str(self.uav_id) + '/mavros/mission/push', service_timeout)
            rospy.wait_for_service('/bomber' + str(self.uav_id) + '/mavros/mission/clear', service_timeout)
            rospy.wait_for_service('/bomber' + str(self.uav_id) + '/mavros/set_mode', service_timeout)
            rospy.loginfo('ROS services are up')
        except rospy.ROSException:
            self.fail('failed to connect to services')

        self.get_param_srv = rospy.ServiceProxy('/bomber' + str(self.uav_id) + '/mavros/param/get', ParamGet)
        self.set_arming_srv = rospy.ServiceProxy('/bomber' + str(self.uav_id) + '/mavros/cmd/arming', CommandBool)
        self.set_mode_srv = rospy.ServiceProxy('/bomber' + str(self.uav_id) + '/mavros/set_mode', SetMode)
        self.wp_clear_srv = rospy.ServiceProxy('/bomber' + str(self.uav_id) + '/mavros/mission/clear', WaypointClear)
        self.wp_push_srv = rospy.ServiceProxy('/bomber' + str(self.uav_id) + '/mavros/mission/push', WaypointPush)
        #-------------
        self.alt_sub = rospy.Subscriber('/bomber' + str(self.uav_id) + '/mavros/altitude', Altitude, self.altitude_callback)
        self.ext_state_sub = rospy.Subscriber('/bomber' + str(self.uav_id) + '/mavros/extended_state', ExtendedState, self.extended_state_callback)
        self.global_pos_sub = rospy.Subscriber('/bomber' + str(self.uav_id) + '/mavros/global_position/global', NavSatFix, self.global_position_callback)
        self.imu_data_sub = rospy.Subscriber('/bomber' + str(self.uav_id) + '/mavros/imu/data', Imu, self.imu_data_callback)
        self.home_pos_sub = rospy.Subscriber('/bomber' + str(self.uav_id) + '/mavros/home_position/home', HomePosition, self.home_position_callback)
        self.local_pos_sub = rospy.Subscriber('/bomber' + str(self.uav_id) + '/mavros/local_position/pose', PoseStamped, self.local_position_callback)
        self.mission_wp_sub = rospy.Subscriber('/bomber' + str(self.uav_id) + '/mavros/mission/waypoints', WaypointList, self.mission_wp_callback)
        self.state_sub = rospy.Subscriber('/bomber' + str(self.uav_id) + '/mavros/state', State, self.state_callback)
        self.target_sub = rospy.Subscriber('/scout' + str(self.scout_id) + '/target_point/position', P, self.goal_pos_callback)

        #test swarm
        self.state_scout0_sub = rospy.Subscriber('/scout' + str(self.scout_id) + '/mavros/state', State, self.state_scout0_callback)
        self.local_scout0_pos_sub = rospy.Subscriber('/scout' + str(self.scout_id) + '/mavros/local_position/pose', PoseStamped, self.local_scout0_position_callback)
        self.scout0_check_sub = rospy.Subscriber('/scout' + str(self.scout_id) + '/mavros/check_mission', PoseStamped, self.scout0_check_callback)
        self.def_prob_sub = rospy.Subscriber('/bomber' + str(self.uav_id) + '/damage', Float32, self.damage_callback)
        self.personal_land_sub = rospy.Subscriber('/bomber' + str(self.uav_id) + '/mavros/personal_land/check_mission', PoseStamped, self.personal_land_callback)
        self.start_mission_sub = rospy.Subscriber('/scout' + str(self.scout_id) + '/start_mission', Bool, self.start_mission_callback)
        self.follower_mode_sub = rospy.Subscriber('/scout' + str(self.scout_id) + '/follower_mode', Bool, self.follower_callback)
        #---
        return
