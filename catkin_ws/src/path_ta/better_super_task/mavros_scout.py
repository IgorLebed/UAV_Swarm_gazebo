...
class MavrosTestCommon(unittest.TestCase):
    ...
    def __init__(self, *args, **kwargs):
        super(MavrosTestCommon, self).__init__(*args, **kwargs)
    ...
    def setUp(self, uav_id):
        ...
        try:
            rospy.wait_for_service('/scout' + str(self.uav_id) + '/mavros/param/get', service_timeout)
            rospy.wait_for_service('/scout' + str(self.uav_id) + '/mavros/cmd/arming', service_timeout)
            rospy.wait_for_service('/scout' + str(self.uav_id) + '/mavros/mission/push', service_timeout)
            rospy.wait_for_service('/scout' + str(self.uav_id) + '/mavros/mission/clear', service_timeout)
            rospy.wait_for_service('/scout' + str(self.uav_id) + '/mavros/set_mode', service_timeout)
        ...
        #------------------------------------Standart_Tipic_List-----------------------------------------------------------------
        #
        self.get_param_srv = rospy.ServiceProxy('/scout' + str(self.uav_id) + '/mavros/param/get', ParamGet)
        self.set_arming_srv = rospy.ServiceProxy('/scout' + str(self.uav_id) + '/mavros/cmd/arming', CommandBool)
        self.set_mode_srv = rospy.ServiceProxy('/scout' + str(self.uav_id) + '/mavros/set_mode', SetMode)
        self.wp_clear_srv = rospy.ServiceProxy('/scout' + str(self.uav_id) + '/mavros/mission/clear', WaypointClear)
        self.wp_push_srv = rospy.ServiceProxy('/scout' + str(self.uav_id) + '/mavros/mission/push', WaypointPush)
        self.alt_sub = rospy.Subscriber('/scout' + str(self.uav_id) + '/mavros/altitude', Altitude, self.altitude_callback)
        self.ext_state_sub = rospy.Subscriber('/scout' + str(self.uav_id) + '/mavros/extended_state', ExtendedState, self.extended_state_callback)
        self.global_pos_sub = rospy.Subscriber('/scout' + str(self.uav_id) + '/mavros/global_position/global', NavSatFix, self.global_position_callback)
        self.imu_data_sub = rospy.Subscriber('/scout' + str(self.uav_id) + '/mavros/imu/data', Imu, self.imu_data_callback)
        self.home_pos_sub = rospy.Subscriber('/scout' + str(self.uav_id) + '/mavros/home_position/home', HomePosition, self.home_position_callback)
        self.local_pos_sub = rospy.Subscriber('/scout' + str(self.uav_id) + '/mavros/local_position/pose', PoseStamped, self.local_position_callback)
        self.mission_wp_sub = rospy.Subscriber('/scout' + str(self.uav_id) + '/mavros/mission/waypoints', WaypointList, self.mission_wp_callback)
        self.state_sub = rospy.Subscriber('/scout' + str(self.uav_id) + '/mavros/state', State, self.state_callback)
        #
        #------------------------------------My_Tipic_List-----------------------------------------------------------------
        #
        self.target_sub = rospy.Subscriber('/scout' + str(self.uav_id) + '/target_point/position', Point, self.goal_pos_callback)
        self.scout0_exit_mode_sub = rospy.Subscriber('/scout' + str(self.uav_id) + '/mavros/check_mission/stop', Bool, self.scout0_exit_mode_callback)
        self.def_prob_sub = rospy.Subscriber('/scout' + str(self.uav_id) + '/damage', Float32, self.damage_callback)
        self.battery_sub = rospy.Subscriber('/scout' + str(self.uav_id) + '/battery', Float32, self.battery_callback)
        self.crit_sit_info_sub = rospy.Subscriber('/critical_status_info', Int64MultiArray, self.crit_sit_info_callback)
        self.start_mission_sub = rospy.Subscriber('/scout' + str(self.uav_id) + '/start_mission', Bool, self.start_mission_callback)
        self.target_find_sub = rospy.Subscriber('/scout' + str(self.uav_id) + '/target_find', Bool, self.target_find_callback)
        return
