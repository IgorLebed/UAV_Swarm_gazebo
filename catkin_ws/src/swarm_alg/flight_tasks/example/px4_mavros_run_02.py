...
import math
import threading
import px4_mavros_run_00 as th

class Px4Controller_2:
    ...
    def __init__(self):
        ...
        self.takeoff_height = (th.Px4Controller_0().takeoff_height)
        ...
        self.mavros_sub = rospy.Subscriber("/uav2/mavros/state", State, self.mavros_state_callback)
        self.gps_sub = rospy.Subscriber("/uav2/mavros/global_position/global", NavSatFix, self.gps_callback)
        self.imu_sub = rospy.Subscriber("/uav2/mavros/imu/data", Imu, self.imu_callback)

        self.set_target_position_sub = rospy.Subscriber("/uav2/gi/set_pose/position", PoseStamped, self.set_target_position_callback)
        self.set_target_yaw_sub = rospy.Subscriber("/uav2/gi/set_pose/orientation", Float32, self.set_target_yaw_callback)
        self.custom_activity_sub = rospy.Subscriber("/uav2/gi/set_activity/type", String, self.custom_activity_callback)


        '''
        ros publishers
        '''
        self.local_target_pub = rospy.Publisher('/uav2/mavros/setpoint_raw/local', PositionTarget, queue_size=10)

        '''
        ros services
        '''
        self.armService = rospy.ServiceProxy('/uav2/mavros/cmd/arming', CommandBool)
        self.flightModeService = rospy.ServiceProxy('/uav2/mavros/set_mode', SetMode)


        print("Px4 Controller2 Initialized!")


    def start(self):
        rospy.init_node("offboard_node_2")
        for i in range(10):
            if self.current_heading is not None:
                break
            else:
                print("uav2 Waiting for initialization.")
                time.sleep(2)
        ...
        if self.takeoff_detection():
            print("Vehicle_2 Took Off!")

        else:
            print("Vehicle_2 Took Off Failed!")
            return
    ...
    def takeoff_detection(self):
        time.sleep(2)
        if self.local_pose.pose.position.z > 0.001 and self.offboard_state and self.arm_state:
            return True
        else:
            return False
