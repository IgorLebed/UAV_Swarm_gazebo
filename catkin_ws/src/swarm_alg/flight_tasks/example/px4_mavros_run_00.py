...
class Px4Controller_0:
    ...
    def __init__(self):
        ...
        self.gps_sub = rospy.Subscriber("/uav0/mavros/global_position/global", NavSatFix, self.gps_callback)
        self.imu_sub = rospy.Subscriber("/uav0/mavros/imu/data", Imu, self.imu_callback)

        self.set_target_position_sub = rospy.Subscriber("/uav0/gi/set_pose/position", PoseStamped, self.set_target_position_callback)
        self.set_target_yaw_sub = rospy.Subscriber("/uav0/gi/set_pose/orientation", Float32, self.set_target_yaw_callback)
        self.custom_activity_sub = rospy.Subscriber("/uav0/gi/set_activity/type", String, self.custom_activity_callback)


        '''
        ros publishers
        '''
        self.local_target_pub = rospy.Publisher('/uav0/mavros/setpoint_raw/local', PositionTarget, queue_size=10)
        ...
        print("Px4 Controller0 Initialized!")


    def start(self):
        rospy.init_node("offboard_node_0")
        for i in range(10):
            if self.current_heading is not None:
                break
            else:
                print("uav0 Waiting for initialization.")
                time.sleep(2)
        ...
        if self.takeoff_detection():
            print("Vehicle_0 Taking Off!")

        else:
            print("Vehicle_0 Taking Off Failed!")
            return
